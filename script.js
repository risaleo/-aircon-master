...
const questions = [{"cat": "basic", "q": "100Vエアコンを200Vコンセントへそのまま接続できる？", "choices": ["できる", "できない"], "answer": 1, "exp": "電圧が異なるため接続できません。"}, {"cat": "basic", "q": "専用回路とは？", "choices": ["延長コード", "エアコンだけが使う電気回路", "アース線", "化粧カバー"], "answer": 1, "exp": "エアコン専用に用意された電気回路です。"}, {"cat": "price", "q": "配管が標準の長さを超えた場合は？", "choices": ["追加なし", "配管延長の追加工事になる", "必ず設置不可", "必ず200Vになる"], "answer": 1, "exp": "標準範囲を超えた配管は追加工事確認が必要です。"}, {"cat": "quality", "q": "真空引きの目的は？", "choices": ["ガスを増やす", "配管内の空気・水分を除去する", "室外機を冷やす", "音を小さくする"], "answer": 1, "exp": "性能低下や故障リスクを防ぎます。"}, {"cat": "talk", "q": "追加料金について適切な説明は？", "choices": ["絶対ありません", "設置状況により追加工事が必要な場合があります", "全部無料です", "確認しません"], "answer": 1, "exp": "設置条件を確認しながら案内します。"}];
...
const wizardQuestions = [
  {key:"replace", q:"買い替えですか？", choices:["買い替え","新規"]},
  ...
  {key:"pipe", q:"標準4mから何mくらい延長？", choices:["なし","1m","2m","3m","4m以上","分からない"]},
...
function startQuiz(cat){hideAll();current=0;score=0;const pool=cat==="today"?[...questions]:questions.filter(x=>x.cat===cat);quizSet=pool.sort(()=>Math.random()-.5).slice(0,10);$("quizTitle").textContent=cat==="today"?"今日の10問":cat==="basic"?"基礎知識":cat==="price"?"工事料金":cat==="quality"?"良質工事":"接客問題";$("quiz").classList.remove("hidden");showQuestion();}
...
function showWizardResult(){
  $("wizardQuestion").textContent="見積もり診断結果";$("wizardChoices").innerHTML="";
  let lines=[], alerts=[];
  ...
  if(wizardAnswers.voltage==="分からない") alerts.push("100V・200V、コンセント形状を確認");
  ...
  if(wizardAnswers.pipe==="4m以上") alerts.push("配管延長は長さ確認後に計算");
  ...
  if(["壁面","屋根置き","天吊","二段置"].includes(wizardAnswers.outdoor)) alerts.push("特殊設置金具・既設金具の確認が必要");
  if(wizardAnswers.floor!=="1階") alerts.push("高所作業・配管経路の確認");
  const total=lines.reduce((s,x)=>s+x.price,0);
  const breakdown=lines.length?`<ul>${lines.map(x=>`<li>${x.name}：<strong>${yen(x.price)}</strong></li>`).join("")}</ul>`:"<p>選択内容では金額加算なし。標準工事内の可能性があります。</p>";
  const talk = lines.length ? "今回は、選択内容から追加工事の可能性があります。特に専用回路・配管長さ・化粧カバーは現地確認で正式金額が決まります。" : "現時点では大きな追加工事は少なそうですが、専用回路・電圧・配管穴は現地確認で確定します。";
